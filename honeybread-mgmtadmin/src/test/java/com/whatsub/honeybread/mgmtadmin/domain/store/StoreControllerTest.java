...
public class StoreControllerTest {
    ...
    void 카테고리목록을_empty하게_요청하면_등록_실패() throws Exception {
        모두_프로퍼티가_정상적으로_입력된_요청이다();
        다음과같은_카테고리목록으로_요청했다(Collections.emptySet());
    ...
    void 결제방식목록을_empty하게_요청하면_등록_실패() throws Exception {
        모두_프로퍼티가_정상적으로_입력된_요청이다();
        다음과같은_결제방식목록으로_요청했다(Collections.emptySet());
    ...
    private void 다음과같은_기본정보로_요청했다(StoreBasicRequest 기본정보_요청) {
        given(스토어_등록요청.getBasic()).willReturn(기본정보_요청);
    }
    ...
    private void 다음과같은_계좌정보로_요청했다(BankAccountRequest 계좌정보_요청) {
        given(스토어_등록요청.getBankAccount()).willReturn(계좌정보_요청);
    }
    ...
    private void 다음과같은_카테고리목록으로_요청했다(Set<Long> 카테고리아이디_목록) {
        given(스토어_등록요청.getCategoryIds()).willReturn(카테고리아이디_목록);
    }

    private void 다음과같은_결제방식목록으로_요청했다(Set<PayType> 결제방식아이디_목록) {
        given(스토어_등록요청.getPayMethods()).willReturn(결제방식아이디_목록);
    }

    private void 모두_프로퍼티가_정상적으로_입력된_요청이다() {
        ...
        다음과같은_카테고리목록으로_요청했다(Set.of(1L));
        다음과같은_결제방식목록으로_요청했다(Set.of(PayType.ACCOUNT_TRANSFER));
    }
